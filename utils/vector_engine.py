...
class VectorGrokkyEngine:
    ...
    def _init_pinecone(self):
        ...
        try:
            ...
            self.index = pinecone.Index(self.pinecone_index)
            ...
            if total_vectors > 0:
                try:
                    ...
                    for i, record in enumerate(sample_records[:5]):
                        ...
                        logger.info(f"  {i+1}. User: {metadata.get('user_id')}, Role: {metadata.get('role')}")
    ...
    async def generate_embedding(self, text):
        ...
        for i in range(self.vector_dimension):
            ...
            vector.append((hash_digest[byte_index] / 255.0) * 2 - 1)  # нормализуем в диапазоне [-1, 1]
    ...
    async def add_memory(self, user_id: str, content: str, role="user"):
        ...
        try:
            ...
            logger.info(f"Добавлена запись в память: user={user_id}, role={role}, id={record_id}")
            return True
