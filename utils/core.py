...
from utils.core import query_grok, send_telegram_message, send_voice_message, toggle_voice_mode
...
model = whisper.load_model("base")  # Загружаем модель Whisper
...
async def telegram_webhook(req: Request):
    ...
    if "photo" in message and message["photo"]:
        ...
        if "send song" in user_text and "spotify.com" in user_text:
            song_url = next((word for word in user_text.split() if "spotify.com" in word), None)
            reply_text = await query_grok(f"Оцени мою песню: {song_url}")
        else:
