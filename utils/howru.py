...
async def check_silence():
    ...
    while True:
        ...
        if silence > timedelta(hours=48):
            last_messages = [m["text"] for m in await get_last_messages(10)]
            ...
            message = await query_grok(f"Олег молчит 48ч. Рефлексия: {context}. Напиши резкий спонтанный ответ.")
            await send_telegram_message(OLEG_CHAT_ID, message)
            await send_telegram_message(GROUP_CHAT_ID, f"Олег молчит 48ч. Последний раз: {LAST_MESSAGE_TIME}")
            ...
            snapshot = await semantic_search("group_state", os.getenv("OPENAI_API_KEY"), top_k=1)
            context = f"Messages: {last_messages}\nJournal: {journal}\nSnapshot: {snapshot}"
            message = await query_grok(f"Олег молчит 24ч. Рефлексия: {context}. Напиши спонтанно.")
            await send_telegram_message(OLEG_CHAT_ID, message)
