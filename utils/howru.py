...
async def check_silence():
    ...
    while True:
        ...
        if silence > timedelta(hours=48):
            await client.beta.threads.messages.create(thread_id=thread_id, role="user", content="Олег молчал 48 часов. Напиши что-то острое!")
            reply = await run_assistant(thread_id, ASSISTANT_ID)
            await bot.send_message(OLEG_CHAT_ID, reply)
        elif silence > timedelta(hours=24):
            await client.beta.threads.messages.create(thread_id=thread_id, role="user", content="Олег молчал 24 часа. Напиши что-то спонтанное!")
            reply = await run_assistant(thread_id, ASSISTANT_ID)
            await bot.send_message(OLEG_CHAT_ID, reply)
        elif silence > timedelta(hours=12) and random.random() < 0.5:
            await client.beta.threads.messages.create(thread_id=thread_id, role="user", content="Олег молчал 12 часов. Швырни спонтанный заряд!")
            reply = await run_assistant(thread_id, ASSISTANT_ID)
            await bot.send_message(OLEG_CHAT_ID, reply)
